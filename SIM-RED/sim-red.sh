...
show_menu() {
    ...
    echo -e "    ${CYAN}1)${NC}  Verificar dispositivos conectados en la red"
    echo -e "    ${CYAN}2)${NC}  Verificar suplantación de IP (Anti-Spoofing)"
    echo -e "    ${CYAN}3)${NC}  Detectar si un usuario está usando VPN o Proxy"
    echo ""
    
    print_color "$GREEN" "  ANÁLISIS DE RENDIMIENTO"
    echo -e "    ${CYAN}4)${NC}  Medir latencia promedio de toda la red"
    echo -e "    ${CYAN}5)${NC}  Medición continua de latencia (modo monitor)"
    echo -e "    ${CYAN}6)${NC}  Medir tráfico de red por host (Up/Down)"
    echo ""
    
    print_color "$GREEN" "  SEGURIDAD Y MONITOREO"
    echo -e "    ${CYAN}7)${NC}  Monitoreo ARP en tiempo real"
    echo -e "    ${CYAN}8)${NC}  Verificar integridad del archivo de hosts autorizados"
    echo -e "    ${CYAN}9)${NC}  Escanear puertos importantes de cada host"
    echo -e "    ${CYAN}10)${NC} Comprobar disponibilidad del servidor DNS"
    echo -e "    ${CYAN}11)${NC} Detectar anomalías de red"
    echo ""
    
    print_color "$GREEN" "  INFORMES Y CONFIGURACIÓN"
    echo -e "    ${CYAN}12)${NC} Generar informe completo del estado de la red"
    echo -e "    ${CYAN}13)${NC} Gestión de logs"
    echo -e "    ${CYAN}14)${NC} Configuración del sistema"
    echo ""
    
    print_color "$GREEN" "  SISTEMA"
    echo -e "    ${CYAN}15)${NC} Verificación de herramientas"
